...
module.exports = {
	...
	execute(message, args){
		...
		if (!args.length){
			...
			// 	})
			// 	.catch(error => {
			// 		message.client.logger.error(`Could not send help DM to ${message.author.tag}.\n`, error);
		...
		if (!command){
			return message.reply("that's not a valid command. Dummy.");
		}
		...
		Embed.addField('is ownerExclusive', `\`${!!command.ownerExclusive}\``, true);
